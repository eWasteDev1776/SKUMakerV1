...
class PdfFormApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.template_folder = get_resource_path('templates')
        print(f"Template folder: {self.template_folder}")  # Debug print
        self.pdf_library = self.load_templates()
        print(f"PDF Library: {self.pdf_library}")  # Debug print
        self.pdf_doc = None
        self.init_ui()
    ...
    def load_templates(self):
        pdf_library = {}
        print(f"Attempting to load templates from: {self.template_folder}")
        if os.path.exists(self.template_folder):
            print(f"Template folder exists: {self.template_folder}")
            files = os.listdir(self.template_folder)
            print(f"Files in template folder: {files}")
            for filename in files:
                if filename.endswith(".pdf"):
                    pdf_path = os.path.join(self.template_folder, filename)
                    if os.path.isfile(pdf_path):
                        print(f"Adding PDF to library: {pdf_path}")
                        pdf_library[os.path.splitext(filename)[0]] = pdf_path
                    else:
    ...
    def load_selected_pdf(self):
        ...
        file_path = self.pdf_library.get(selected_sheet)
        if file_path:
            self.open_pdf(file_path)
        else:
            print(f"Selected PDF template not found: {selected_sheet}")

    def open_pdf(self, file_path):
        print(f"Attempting to open PDF: {file_path}")  # Debug print
        try:
            self.pdf_doc = fitz.open(file_path)
            if self.pdf_doc and len(self.pdf_doc) > 0:
                print(f"Successfully opened PDF with {len(self.pdf_doc)} pages")  # Debug print
                self.load_form_fields(self.pdf_doc)
            else:
                print("PDF document is empty or could not be loaded")  # Debug print
